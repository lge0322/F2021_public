_module = 'cs112_f21_week6_linter.py version 1.1'
...
'''
    ...
    def lintTopLevel(self):
        ...
        for topLevelNodeList in self.astList:
            ...
            if not ((text in ['@', 'import', 'from', 'def',
                    'class', 'top-level-string'])
                    or self.topLevelPermitted(topLevelNodeList, 
                                            ['make_dataclass',
                                             'SimpleNamespace'])):
    ...
    def topLevelPermitted(self, nodeList, tokens):
        for t in tokens:
            if self.recursiveSearch(nodeList, t):
                return True
        return False

    def recursiveSearch(self, L, s, maxDepth = 12):
        if maxDepth<=0 or not isinstance(L, list):
            return L == s
        else:
            for item in L:
                if self.recursiveSearch(item, s, maxDepth - 1) == True:
                    return True
            return False
